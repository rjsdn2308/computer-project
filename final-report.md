...
| 분야        | 기술/서비스         | 특징                  | 한계                               |
| --------- | -------------- | ------------------- | -------------------------------- |
| 과일 인식(CV) | Fruits-360 등   | 연구용 데이터 기반 과일 분류 가능 | “수확 전 모습”·“자라는 방식” 안내는 부족        |
| 교육용 LLM   | GPT API 기반 Q&A | 맞춤형 질문 답변 가능        | 유아용 말투, 짧은 문장, OX 퀴즈 통합 사례 거의 없음 |
| 유아 앱      | 키즈 학습앱         | 그림·음성 기반            | 상호작용 및 탐구학습 부족                   |

**본 프로젝트의 차별성**

* CV + LLM 결합한 유아 맞춤형 앱
* 쉬운 말투·짧은 문장·친절한 안내
* 수확 전 과일 이미지 제공 → 시각 학습 강화
* OX 퀴즈 포함한 반복 학습 구조
* 파스텔 톤·단순 UI → 유아 친화적 환경

#### 2.2.3 선행 기술 한계 및 개선점

| 기존 한계          | 개선 내용                                       |
| -------------- | ------------------------------------------- |
| 과일 데이터셋 연구용 한정 | 직접 수집한 실제 사진 기반 모델 구축                       |
| 어린이 대상 설명 어려움  | 유아 수준에 맞춘 부드러운 LLM 프롬프트 설계                  |
| 단순 이미지 분류      | 자라는 방식(tree/ground/vine) + 수확 전 이미지 + 설명 제공 |
| 학습 요소 부족       | O/X 퀴즈, 감성 재작성 기능 포함                        |
